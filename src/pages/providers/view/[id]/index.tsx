...
import { deleteHealthCoachById, createHealthCoach } from 'apiSdk/health-coaches';
...
function ProviderViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<ProviderInterface>(
    ...
    () =>
      getProviderById(id, {
        relations: ['user', 'course', 'health_coach'],
      }),
  );
  ...
  const [health_coachUserId, setHealth_coachUserId] = useState(null);
  const health_coachHandleCreate = async () => {
    setCreateError(null);
    try {
      await createHealthCoach({ provider_id: id, user_id: health_coachUserId });
      setHealth_coachUserId(null);
      await mutate();
    } catch (error) {
      setCreateError(error);
    }
  };
  const health_coachHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await deleteHealthCoachById(id);
      await mutate();
    } catch (error) {
      setDeleteError(error);
    }
              ...
              {hasAccess('course', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/courses/create?provider_id=${data?.id}`}>
            ...
            <Box>
              <Stack spacing={2} mb={8}>
                ...
                <Flex gap={5} alignItems="flex-end">
                  <Box flex={1}>
                    <UserSelect
                      name={'health_coach_user'}
                      value={health_coachUserId}
                      handleChange={setHealth_coachUserId}
                    />
                  </Box>
                  <Button
                    colorScheme="blue"
                    mt="4"
                    mr="4"
                    onClick={health_coachHandleCreate}
                    isDisabled={!health_coachUserId}
                  >
                    Create
                  </Button>
                </Flex>
                          ...
                          <Td>
                            {hasAccess('user', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                              <IconButton
                                onClick={(e) => {
                                  e.stopPropagation();
                                  health_coachHandleDelete(record.id);
                                }}
                                colorScheme="red"
                                variant="outline"
                                aria-label="edit"
